...
==== Associated Operations
* <<Answer-prompt>> {nbsp}
* <<Answer-prompt-by-model>> {nbsp}
* <<Define-prompt-template>> {nbsp}
* <<Run-with-memory>> {nbsp}
...
=== Create Text Embedding
`<langchain:create-text-embedding>`
...
==== For Configurations.
* <<embedding-model-configuration>> {nbsp}
...
[[Answer-prompt-by-model]]
=== Answer Prompt By Model
`<langchain:answer-prompt-by-model>`
